# Product

## Register

brand

## Platform

web

## Users

Developers evaluating a starter template for their next project, arriving from GitHub or a blog link. They work with coding agents (Claude Code, Codex) daily, are skeptical of AI-generated code and AI-generated marketing alike, and decide in under a minute whether a starter is worth cloning. Secondary audience: the same person six months later, reading the blog for conventions and patterns.

## Product Purpose

A Rails 8 starter wired for AI-assisted development: Inertia + React frontend, auth, jobs, CI, and deploys ready, plus the part most starters skip — committed agent instructions (CLAUDE.md / AGENTS.md) and project skills that teach an agent the codebase's conventions before it writes a line. Success: a visitor clones the repo or reads a post, and their agent produces code they'd actually merge.

## Positioning

The quality of AI-written code is set by the codebase it lands in. This starter ships the briefing, not just the boilerplate.

## Conversion & proof

- Primary CTA: clone the repository / use it as a GitHub template.
- Secondary CTA: read the blog.
- The line a visitor remembers after 10 seconds: "your agent has already read this codebase."
- Belief ladder: (1) agent output quality depends on the repo, not the model; (2) this repo encodes real, opinionated conventions the agent follows; (3) the stack underneath is boring and proven, so it survives past the demo; (4) it's a clone away.
- Proof on hand: the repo itself — real CLAUDE.md rules, nine committed skills, real Gemfile versions. No testimonials; the artifacts are the proof, quoted verbatim.

## Brand Personality

Opinionated, dry, exact. A well-run print shop: heavy black type, one bright spot color, nothing decorative that isn't doing work. The page speaks in the same voice as the repo's own instruction files — confident rules, stated flatly, occasionally funny because they are true.

## Anti-references

- Gradient-mesh AI-SaaS landing pages: glassmorphism, glowing orbs, "✨ AI-powered" badges and pill chips, fake logo walls, hero metrics.
- Terminal-cosplay dev-tool dark mode (green-on-black, fake shell prompts as decoration).
- Editorial-magazine affectation: italic display serifs, drop caps, eyebrow labels above every section.
- Anything a visitor could identify as AI-generated at a glance.

## Design Principles

- The repo is the imagery. Real files, real rules, real versions — quoted verbatim, typeset like they matter. Never invent proof.
- Practice what you preach: the page is served by the Rails app it describes, styled by the app's own Tailwind theme and typeface.
- One spot color, committed. Ink, paper, and pink doing all the brand work, like a two-ink riso print.
- Say less, mean it. Every sentence is a claim someone could check by opening the repo.

## Accessibility & Inclusion

WCAG AA: body text ≥ 4.5:1, large display ≥ 3:1, visible focus states, reduced-motion alternatives for any entrance motion, semantic landmarks and heading order. Server-rendered HTML; readable without JavaScript.
